...
- 可选：`go test ./auth/` 验证单元测试（模板渲染、DTLS DTD 解析）

## 平台说明

- 认证、隧道、DTLS、动态分流等核心功能**全平台有效**（Windows / Linux / macOS / Android / iOS），`device-id` 平台标识按系统自动映射（`win` / `linux-64` / `mac-intel` / `android` / `apple-ios`）
